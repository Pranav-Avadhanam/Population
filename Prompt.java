...
public class Prompt
{
	...
	public static char getChar (String ask)
	{
			...
			while(!found)
			{
				...
				{
					System.err.println("ERROR: Single character string must be inputted");
					found = false;
				}
			}
			return val;
	...
	public static int getInt (String ask)
	{
			...
			while(!found)
			{
				...
				{
					found = false;
					System.err.println("ERROR: Single integer must be inputted");
				}
			}
			return val;
	}
	...
	{
			int val = 0;
			val = getInt(ask);//val = getInt(ask + "("+min+","+max+")");
			while(val < min || val>max)
			{
					System.err.println("ERROR: Input integer must be within bounds");
					val = getInt(ask);//val = getInt(ask + "("+min+","+max+")");
			}
			return val;
	}
	...
	public static double getDouble (String ask, double min, double max)
	{
			...
			while(val < min || val>max)
			{
					System.err.println("ERROR: Input double must be within bounds");
					val = getDouble(ask + "("+min+","+max+")");
			}
			return val;
	}
